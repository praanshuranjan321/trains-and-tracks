...
## [2026-04-18 09:52 IST] 7ebb95b: fix(retry) split release_hold terminal vs retryable + investigate hold_expired origin
- context: 100-req surge produced 55 CONFIRMED + 22 FAILED + 25 EXPIRED + 13 DLQ vs the ~97-99 CONFIRMED that PAYMENT_FAILURE_RATE=0.3 × 3 retries should yield (P_permanent = 0.3³ = 2.7%). Two interlocking bugs. O4: the old `release_hold(booking_id, reason)` SP stamped booking.status=FAILED unconditionally; on next QStash delivery the worker's step-3 "if booking.status != 'PENDING' return" guard treated it as already-terminal and returned 200, burning the retry budget silently. Effective retries = 0. O1: the non-PaymentError catch branch returned 500 without any release_hold call; the seat stayed RESERVED until the 60-s sweeper expired it 5 min later (appearing as "25 EXPIRED with failure_reason=hold_expired"). Prod psql probe confirmed TTL is correctly 300s — not a unit bug
- decision: split release_hold into two idempotent SPs in new migration 20260418000000_split_release_hold.sql. `release_hold_retryable(uuid)` clears the seat only (booking stays PENDING for the next QStash delivery). `release_hold_terminal(uuid, text)` clears seat + marks booking FAILED (permanent branches only). Old `release_hold` kept with deprecation COMMENT for backward compat. Repository + hold-state-machine surface both variants. Worker payment branch rewritten per mandate: payment_declined → terminal + 489; payment_timeout (attempts remaining) → retryable + 500; payment_timeout (last attempt) → terminal + 489; non-PaymentError with attempts remaining → retryable + 500 (fixes O1 seat stranding); non-PaymentError last attempt → terminal + 489; sold_out → terminal. run-worker-job.ts parity for the local-bypass queue. Webhook `/api/webhooks/qstash-failure` adds safety-net cleanup: decodes the original payload, calls release_hold_terminal + commits FAILED idempotency body — closes the tombstone gap where QStash exhausts retries against 500s but the worker never went terminal. Migration applied to prod DB directly so the deploy lands on live functions
- files: supabase/migrations/20260418000000_split_release_hold.sql, lib/db/repositories/bookings.ts, lib/allocation/hold-state-machine.ts, app/api/worker/allocate/route.ts, lib/allocation/run-worker-job.ts, app/api/webhooks/qstash-failure/route.ts

## [2026-04-18 10:09 IST] a56898d: refactor(ops) drop Grafana iframe (public-share template-var incompatibility); metrics pipeline unchanged
- context: Grafana Cloud free/trial tier ships `X-Frame-Options: deny` on public dashboards AND ignores `$env` template variables on the public-share route — so the earlier iframe was unusable, and the CTA-card replacement (1a51e39) landed on the same two problems once opened. Neither shortcoming is fixable from our code without a paid-tier upgrade
- decision: replace the CTA card with a compact "METRICS PIPELINE" status card below the Recharts hero. Shows backend identity ("Grafana Cloud Mimir · env=production") + last-sample-age as an end-to-end liveness signal, computed read-only from the existing /api/insights proxy's latest-point timestamp (not a literal waitUntil-fired-at-T, but proves the full chain registry → pusher → remote_write → Mimir → insights is alive). Scope strictly limited per mandate: pusher.ts / registry.ts / insights route / Vercel env vars all untouched — pipeline still pushes in the background; `NEXT_PUBLIC_GRAFANA_DASHBOARD_URL` stays set for future re-embed if we ever get iframe-embed on a paid tier. Zero test traffic fired to verify — build-only gate

## [2026-04-18 11:15 IST] polish/editorial-hero branch (d06d13a..eb10790): Phase 7 landing polish — editorial travel-magazine direction + motion
- context: Phase 7 polish budget (5:15 → 10:30 IST). Brief was to elevate the landing hero (Pexels-photo + Anton + signal-green → travel-magazine editorial) without touching /lib, /app/api, /supabase, /infra, /scripts. /ops got a sub-scope tighten so it fits one viewport.
- decision: swapped Anton display face → Fraunces (variable serif, italic) via next/font/google; renamed next/font CSS variables to --font-inter / --font-jetbrains-mono / --font-fraunces to fix the pre-existing `--font-mono: var(--font-geist-mono)` self-reference in globals.css @theme. Added editorial palette tokens (sky-teal, sky-cyan, cloud-ivory, train-red, train-red-dark, bridge-stone, mountain-deep, ink, paper) + preserved signal-green / dark-bg explicit tokens for /ops. Hero revised twice — first to an asymmetric mounted-plate editorial composition (d06d13a), then per taste redirect to full-bleed cover (b46dd95), then overlay-free with per-glyph text-shadow + framer-motion staggered reveal + GSAP ScrollTrigger image parallax (eb10790, client component w/ useGSAP from @gsap/react). a11y follow-up pass after P1 (506d471) added focus-visible rings, motion-reduce guards, explicit transition-property lists, text-balance, aria-hidden on decorative icons, translate="no" on the wordmark, colorScheme: dark. P2 cascade (b46dd95) rewrote Problem / Six Promises / Three Defense / CTA strip / Footer with hairline-rule composition, serif display numerals (Fraunces opsz=144 SOFT=100 WONK=1), italic serif captions, magazine-colophon footer with four link columns. LoadingScreen mounted in layout — letter-by-letter brand wordmark, hairline progress bar, rotating mono caption, 1600ms minimum dwell, AnimatePresence fade-out on window load. /ops tightened (6eb8043) to h-[calc(100vh-40px)] overflow-hidden flex flex-col with chart flex-1 — superseded ~30 min later by 1cf2754's single-viewport 2-column redesign. /book palette shift (P3) deferred — WIP statusColor edit was reverted externally; train-red selected / cloud-ivory available / bridge-stone confirmed treatment not shipped in this pass.
- files: app/layout.tsx, app/globals.css, app/page.tsx, app/ops/page.tsx (tighten only), components/landing/HeroSection.tsx, components/landing/LoadingScreen.tsx, docs/DECISIONS.md

## [2026-04-18 11:32 IST] 48b672c: perf(qstash) raise flow-control parallelism 1 → 10 per train
- context: observed ~0.5-2 confirmations/sec per train during the 100-req surge — 2 min drain for 100 bookings is too slow for a demo chart to feel alive. Per-train throughput was bottlenecked by parallelism=1 serialization, not by any correctness need
- decision: raise parallelism 1 → 10 on the `train.{id}` flow-control key. Correctness is rate-independent and protected by three already-in-place mechanisms: FOR UPDATE SKIP LOCKED in allocate_seat (migration 100:45), allocate_seat re-delivery guard (migration 100:17-25), bookings.idempotency_key UNIQUE backstop (migration 060 / ADR-024). I1 and I2 invariants hold at any parallelism. Tradeoff: strict per-train FIFO relaxed (booking #3 can confirm before #2) — acceptable; real IRCTC doesn't guarantee FIFO either. Ceiling for this stack is ~50-100 before Supavisor 200-slot pool saturates and Cockatiel breaker trips; past that, throughput collapses (Little's Law: concurrent = arrival × service_time; service time ≈1s so 200 = pool ceiling). Scale past single-train ceiling is "add more trains" — each flow-control key is a parallelism-bounded lane
- files: infra/qstash/publisher.ts

## [2026-04-18 11:45 IST] a5a3d50: perf(qstash) raise parallelism 10 → 50 for demo-day 100-req surge
- context: demo target is a 100-req burst. At parallelism=10 drain takes ~15s with visible sawtooth; operator wants the chart to feel instantaneous when they click SIMULATE
- decision: raise parallelism 10 → 50. At 50 concurrent workers × ~1s service time, 100 bookings drain in ~2-3s across two QStash batches. Pool utilization: 50/200 Supavisor slots = 25%, well under the saturation cliff at ~150-180. Cockatiel breaker unreachable at PAYMENT_FAILURE_RATE=0.05 (50% failure over 10s required to trip; probability effectively zero at n=100 with p=0.05). Correctness unchanged — same three guarantees (SKIP LOCKED, re-delivery guard, UNIQUE). Stays within the stack's 50-100 ceiling identified in the 48b672c analysis; past 100 we'd want to add trains, not raise this further
- files: infra/qstash/publisher.ts

## [2026-04-18 10:42 IST] 1cf2754: refactor(ops) single-viewport 2-column dashboard — no scroll during surge demo
- context: the operator had to scroll down to reach SIMULATE SURGE, which meant clicking it scrolled away from the chart — the entire reason to click was to watch the chart light up, defeating the demo. The prior chartRef scrollIntoView workaround (6eb8043) patched the symptom; this fixes the structure.
- decision: full-viewport layout (h-screen + overflow-hidden) with a 12-col grid: LEFT 9 cols = hero chart (flex-1) + 6 KPI tiles (compact strip) + last-10 bookings (fixed max-height, internal scroll); RIGHT 3 cols = admin secret + surge card + chaos (kill+reset) + DLQ live panel (fills remaining height, internal scroll). Footer appears only when lastAction is set. DLQ moves from click-to-log to a 10s-poll live panel — /api/admin/dlq gains kind:'read' so it uses the 300/min READ bucket (ADR-011) and doesn't starve mutations. Dropped p95/p99 tiles from the viewport (still pushed to Grafana), the HUD dashed-line SVG (no longer geometrically meaningful), and the old "DLQ list" click-button. MiniStat gained a `compact` prop for tighter 6-tile strip; legacy wide/raw/accent props untouched. Metrics pipeline (pusher / registry / insights / Vercel env vars) unchanged. Build-only gate, zero traffic fired
- files: app/ops/page.tsx, app/api/admin/dlq/route.ts
